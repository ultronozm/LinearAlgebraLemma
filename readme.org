...
Let M_{n} denote the space of complex n \times n matrices, included in the space M_{n+1} of (n+1) \times (n+1) matrices as the upper-left block.  Let τ be an element of M_{n+1} with the property that no eigenvalue of τ is also an eigenvalue of the upper-left n \times n submatrix of τ.  Let x be an element of M_{n+1} with [x,\tau] = 0.  Let z denote the image in M_{n+1} of the identity element of M_{n}, thus z is a diagonal matrix with entries (1, ..., 1, 0).  Suppose that [x,[z,τ]] = [y, τ] for some y in M_{n}.  Then x is a scalar matrix.
